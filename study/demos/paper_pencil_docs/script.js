(function () {
  ...
  const paperSamples = {
    write: "assets/pencil_write_bigsoundbank.mp3",
    erase: "assets/ogg/pencil_erase.ogg",
    move: "assets/paper_move_luckius.mp3"
  };
  ...
  function playSample(src, options = {}) {
    if (!settings.earcons) return;

    const audio = new Audio(src);
    const volume = options.volume == null ? 0.75 : options.volume;
    ...
    const startPlayback = () => {
      const beginDirectPlayback = () => {
        const playPromise = audio.play();
        if (playPromise) {
          playPromise.catch((error) => {
            addActivity(`Audio sample could not play: ${error.name}.`);
          });
        }
      };
      ...
      beginDirectPlayback();
      scheduleSampleStop(audio, options.duration);
    };

    if (options.delay) {
      window.setTimeout(startPlayback, options.delay);
    } else {
      startPlayback();
    }
  }

  function scheduleSampleStop(audio, duration) {
    if (!duration) return;
    window.setTimeout(() => {
      audio.pause();
      audio.src = "";
  ...
  function reportSampleSupport() {
    if (prototype !== "paper") return;

    const audio = document.createElement("audio");
    const canPlayMp3 = audio.canPlayType("audio/mpeg");
    const canPlayOgg = audio.canPlayType("audio/ogg; codecs=vorbis");
    if (!canPlayMp3) {
      addActivity("This browser did not report MP3 support; paper/pencil writing samples may not play.");
    }
    if (!canPlayOgg) {
      addActivity("This browser did not report OGG Vorbis support; erase samples may not play.");
    }
  }

  function playPaperCue(collaborator, options = {}) {
    const spatialOptions = getCollaboratorSpatialOptions(collaborator);
    const pan = options.pan == null ? spatialOptions.pan : options.pan;
    const sample = options.kind === "erase"
      ? paperSamples.erase
      : options.kind === "move"
        ? paperSamples.move
        : paperSamples.write;
  ...
  function playEditEarcon(collaborator, editType) {
    ...
    if (prototype === "paper") {
      playPaperCue(collaborator, {
        kind: editType === "delete" ? "erase" : "write",
        duration: editType === "delete" ? 900 : 1200,
        volume: editType === "delete" ? 0.62 : 0.68
      });
      return;
    }
  ...
  function playInteractionEarcon() {
    ...
    if (prototype === "paper") {
      playSample(paperSamples.write, { volume: 0.64, duration: 1000, startAt: 0 });
      return;
    }
  ...
  function drainQueue() {
    ...
    window.setTimeout(() => {
      ...
      const shouldPlayIdentity = ["join", "nearby", "userAway", "collabMoved", "leave"].includes(item.eventType);
      const paperMovementEvent = prototype === "paper" && ["userAway", "collabMoved"].includes(item.eventType);
      if (shouldPlayIdentity && !paperMovementEvent) {
  ...
  function joinMessage(collaborator) {
    if (prototype === "playful") return `${collaborator.name} hopped onto the document in ${sectionNames[collaborator.location.para]}.`;
    if (prototype === "paper") return `${collaborator.name} joined in ${sectionNames[collaborator.location.para]}.`;
  ...
  function simulateCollaboratorEdit(collaborator, editType) {
    ...
    const message = editType === "delete"
      ? `${collaborator.name} erased text in ${editedSection}.`
  ...
  function scheduleMove(collaborator) {
    ...
    setDemoTimer(() => {
      ...
      if (settings.earcons && prototype !== "functional") {
        if (prototype === "paper") {
          playPaperCue(collaborator, { kind: "move", pan: oldPan, duration: 700, volume: 0.6 });
          playPaperCue(collaborator, { kind: "move", delay: 520, duration: 700, volume: 0.6 });
        } else {
          ensureAudio();
          const now = audioContext.currentTime + 0.02;
      ...
      if (currentCursor.para === originPara) {
        enqueueAnnouncement(`${collaborator.name} moved to ${sectionNames[collaborator.location.para]}.`, collaborator, "collabMoved");
      }
    }, 8500);
  }
    ...
    getVisibleCollaborators().forEach((collaborator) => {
      ...
      if (samePara && !previous.para) {
        const message = prototype === "functional"
          ...
          : prototype === "paper"
            ? `${collaborator.name} is writing in this paragraph.`
            : `${collaborator.name} is close by in this paragraph.`;
        enqueueAnnouncement(message, collaborator, "nearby");
        scheduleMove(collaborator);
      }
      if (sameLine && !previous.line) {
        enqueueAnnouncement(`Same line as ${collaborator.name}.`, collaborator, "sameLine");
      }
      if (!samePara && previous.para) {
        enqueueAnnouncement(`Moved away from ${collaborator.name}.`, collaborator, "userAway");
        if (prototype === "paper") {
          playPaperCue(collaborator, { kind: "move", duration: 520, volume: 0.48 });
        }
      }
  ...
  function playOverviewEarcons() {
    ...
    Object.keys(sectionNames).forEach((id, index) => {
      ...
      if (prototype === "paper") {
        playSample(paperSamples.write, {
          delay: index * 130,
          duration: 420 + count * 80,
          volume: 0.42,
          startAt: 0,
          rate: 0.92 + count * 0.025,
          pan: -0.7 + index * 0.28
        });
      } else {
  ...
  [speechToggle, earconToggle, panningToggle, activityToggle, densitySelect].forEach((control) => {
